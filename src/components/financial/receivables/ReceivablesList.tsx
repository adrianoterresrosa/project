...
interface ReceivablesListProps {
  ...
  onEdit: (receivable: AccountsReceivable) => void;
  onDelete: (id: string) => void;
}

export function ReceivablesList({ receivables, onEdit, onDelete }: ReceivablesListProps) {
  ...
  const filteredReceivables = receivables.filter(receivable => {
    const matchesSearch = 
      ...
      receivable.document_number?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      receivable.partner_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      receivable.partner_document?.includes(searchQuery);

    const matchesStatus = statusFilter === 'all' || 
      (statusFilter === 'open' && receivable.remaining_amount > 0) ||
      (statusFilter === 'paid' && receivable.remaining_amount === 0);

    const date = dateFilter === 'issue' ? receivable.issue_date : receivable.next_due_date;
  ...
  const calculateStatus = (receivable: AccountsReceivable) => {
    if (!receivable.total_installments) return 'Sem parcelas';
    if (receivable.remaining_amount === 0) return 'Pago';
    if (receivable.paid_installments === 0) return 'Em aberto';
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-sm font-medium text-gray-500 border-b border-gray-200">
                <th className="pb-3 px-6">Cliente</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredReceivables.map((receivable) => {
                const status = calculateStatus(receivable);
                const statusColor = getStatusColor(status);

                return (
                  <tr key={receivable.id} className="text-sm text-gray-800">
                    <td className="py-4 px-6">{receivable.partner_name}</td>
                    ...
                    <td className="py-4 px-6">{receivable.description}</td>
                    <td className="py-4 px-6">{formatDate(receivable.issue_date)}</td>
                    <td className="py-4 px-6">
                      {receivable.next_due_date ? formatDate(receivable.next_due_date) : '-'}
                    </td>
                    <td className="py-4 px-6 text-right font-medium">
                      <div className="flex items-center justify-end">
                        <DollarSign className="h-4 w-4 text-gray-400 mr-1" />
                        {formatCurrency(receivable.gross_amount || receivable.total_amount)}
                      </div>
                    </td>
                    <td className="py-4 px-6 text-right font-medium text-green-600">
                      {formatCurrency(receivable.total_paid_amount || 0)}
                    </td>
                    <td className="py-4 px-6 text-right font-medium text-blue-600">
                      {formatCurrency(receivable.remaining_amount || 0)}
                    </td>
                    <td className="py-4 px-6">
